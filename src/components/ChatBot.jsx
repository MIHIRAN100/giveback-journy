...
const ChatBot = ({ cookieVisible, isTourDetails }) => {
    ...
    useEffect(() => {
        ...
        // Listen for Tidio events to hide our custom button when Tidio is open
        const setupTidioListeners = () => {
            if (window.tidioChatApi) {
                window.tidioChatApi.on('open', () => setIsTidioOpen(true));
                window.tidioChatApi.on('close', () => {
                    setIsTidioOpen(false);
                    // Re-hide the tidio default button when closed, so ours shows again
    ...
    if (isTidioOpen) return null; // Don't show custom button if Tidio window is open
